...
int				MajorNo,Val=-1,i;	
static	unsigned short		*Base2=NULL;
	unsigned int		BAR[5];
...
static int device_ioctl(struct inode *inode,struct file *file,unsigned int ioctl_num,unsigned long ioctl_param)
{
	...
	case IOCTL_CARD1_MEM_READ_MODULE1:
		{
			...
			{
				unsigned short Data;
				Data	=	readw((Base2)+ioctl_param);
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_READ_MOD_1 : ADDR: %08X,Data: %X\n",((unsigned int)(Base2+ioctl_param)),Data);
				return (Data);
			}
			else
				return (-1);
	...
	case IOCTL_CARD1_MEM_WRITE_MODULE1:
		{
			...
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_WRITE_MOD_1 : ADDR: %X, DATA: %X\n",((unsigned int)(Base2+ptr->Addr)),(ptr->Data));
				writew(ptr->Data,(Base2)+(ptr->Addr));
			}
		}
		break;
	...
	case IOCTL_CARD1_REG_READ_MODULE1:
		{
			...
			{
				unsigned short Data;
				Data	=	readw(Base2+(0x80000)+ioctl_param);
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_REG_READ_MOD_1: Addr :%08X,Data :%04X",((unsigned int)(Base2+(0x80000)+ioctl_param)),Data);
				return (Data);
			}
			else 
				return (-1);
	...
	case IOCTL_CARD1_REG_WRITE_MODULE1:
		{
			...
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_REG_WRITE_MOD_1: Addr :%08X,Data :%04X",((unsigned int)(Base2+(0x80000)+ptr->Addr)),(ptr->Data));
				writew(ptr->Data,Base2+(0x80000)+(ptr->Addr));
			}
		}
		break;
	...
	case IOCTL_CARD1_MEM_READ_MODULE2:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x100000)+ioctl_param);
			if((ioctl_param >= 0x0) && (ioctl_param <= 0x7FFFF))
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_READ_MOD_2 : ADDR: %X,Data: %X\n",((unsigned int)(Base2+(0x100000)+ioctl_param)),Data);
				return (Data);
			}
			else 
				return -1;
	...
	case IOCTL_CARD1_MEM_WRITE_MODULE2:
		{
			...
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_WRITE_MOD_2 : ADDR: %X, DATA: %X\n",((unsigned int)(Base2+(0x100000)+ptr->Addr)),(ptr->Data));
				writew(ptr->Data,Base2+(0x100000)+(ptr->Addr));
			}
		}
		break;
	...
	case IOCTL_CARD1_REG_READ_MODULE2:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x180000)+ioctl_param);

			if((ioctl_param >= 0x0) && (ioctl_param <= 0x7FFFF))
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_REG_READ_MOD_2 : Addr :%08X,Data :%04X",((unsigned int)(Base2+(0x180000)+ioctl_param)),Data);
				return (Data);
			}
			else 
				return -1;
	...
	case IOCTL_CARD1_REG_WRITE_MODULE2:
		{
			...
			if((ptr->Addr >= 0x0) && (ptr->Addr <= 0x7FFFF))
				writew(ptr->Data,Base2+(0x180000)+(ptr->Addr));
		}
		break;
	...
	case IOCTL_CARD1_MEM_READ_MODULE3:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x200000)+ioctl_param);

			if((ioctl_param >= 0x0) && (ioctl_param <= 0x7FFFF))
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_READ_MOD_3 : ADDR: %X,Data: %X\n",((unsigned int)(Base2+0x200000+ioctl_param)),Data);
				return (Data);
			}
			else 
				return -1;
	...
	case IOCTL_CARD1_MEM_WRITE_MODULE3:
		{
			...
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_WRITE_MOD_3 : ADDR: %X, DATA: %X\n",((unsigned int)(Base2+(0x200000)+ptr->Addr)),(ptr->Data));
				writew(ptr->Data,Base2+(0x200000)+(ptr->Addr));
			}
		}
		break;
	...
	case IOCTL_CARD1_REG_READ_MODULE3:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x280000)+ioctl_param);
	...
	case IOCTL_CARD1_REG_WRITE_MODULE3:
		{
			...
			if((ptr->Addr >= 0x0) && (ptr->Addr <= 0x7FFFF))
				writew(ptr->Data,Base2+(0x280000)+(ptr->Addr));
		}
		break;
	...
	case IOCTL_CARD1_MEM_READ_MODULE4:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x300000)+ioctl_param);

			if((ioctl_param >= 0x0) && (ioctl_param <= 0x7FFFF))
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_READ_MOD_4 : ADDR: %X,Data: %X\n",((unsigned int)(Base2+0x300000+ioctl_param)),Data);
				return (Data);
			}
			else 
				return -1;
	...
	case IOCTL_CARD1_MEM_WRITE_MODULE4:
		{
			...
			{
				if(CARD1_DEBUG_FLAG)
					printk("APOORVA_PCI: CARD1_MEM_WRITE_MOD_4 : ADDR: %X, DATA: %X\n",((unsigned int)(Base2+(0x300000)+ptr->Addr)),(ptr->Data));					
				writew(ptr->Data,Base2+(0x300000)+(ptr->Addr));
			}
		}
		break;
	...
	case IOCTL_CARD1_REG_READ_MODULE4:
		{
			unsigned short Data;
			Data	=	readw(Base2+(0x380000)+ioctl_param);
	...
	case IOCTL_CARD1_REG_WRITE_MODULE4:
		{
			...
			if((ptr->Addr >= 0x0) && (ptr->Addr <= 0x7FFFF))
				writew(ptr->Data,Base2+(0x380000)+(ptr->Addr));
		}
		break;
...
static int __init APOORVA_CARD1_INIT(void)
{
	...
	if(ret)
	{
		printk("Warning--------> Problem While Enabling the Divice \n ret  is :%04X\n",ret);
	} 
	...
	//pci_init();
	//printk("\nAPOORVA_PCI: APOORVA1553 Device bars Has Been  enables =%d\n",pci_enable_device_bars(dev_APOORVA,BAR ));
	//printk("Warning--------> Probe irq returns :%04X\n",probe_irq_on());
	...
	printk("[0x%1X]\n",IntrLine);
	printk("APOORVA_PCI: READING PCI IRQ PIN.............");	pci_read_config_byte(dev_APOORVA ,PCI_INTERRUPT_PIN,&IntrPin);		printk("[0x%04X]\n",IntrPin);

	/*for(ret=0;ret<=5;ret++)
	{
		printk("APOORVA_PCI: READING PCI BASE %d...............[0x%4X] \n",ret,dev_APOORVA->resource[ret]);
		
	}*/
	//printk("APOORVA_PCI:Irq before Allocation......%d.......pin[0x%2X] \n",dev_APOORVA->irq,dev_APOORVA->pin);
	//printk("APOORVA_PCI: READING PCI rom_base_addr........[0x%4X]\t rom_size [0x%8X]\n",dev_APOORVA->rom_base_reg,dev_APOORVA->cfg_size);
...
/*	ret = pci_enable_device(dev_APOORVA );
	...
	//ret=2;
	// pci_fill_info(dev_APOORVA, ret);APOORVA1553
	printk("APOORVA_PCI:Irq Allocation......%d.......pin[0x%2X] \n",dev_APOORVA->irq,dev_APOORVA->pin);

	 Base0 = (unsigned char *)CheckAndAllocateMemory(BAR[0],0x60);
//	Val	=	request_irq(IRQL,ISR,SA_SHIRQ,"APOORVA1553",irqdevid);  //For SMP
	/*Val	=	request_irq(IRQL,(void *)&ISR,SA_SHIRQ,"APOORVA1553",irqdevid);  //For SMP///vsr
	//printk("RetVal of request_irq() is :%04X\n",Val);
	if(Val)
	{
		printk("\tIrq Allocation Failure\n");
		return 0;
	...
	/*for(i=11;i<=255;i++)
	{
		Val	=	request_irq(i,ISR,SA_SHIRQ,"APOORVA1553",irqdevid);
		//Val	=	request_irq(i,ISR,IRQF_SHARED,"APOORVA1553",irqdevid);
		printk("RetVal of request_irq() is :%04X\n",Val);
		if(Val==0)
			break;
	}*/
	...
	if(Base2 == '\0')
	{
		...
		//ree_irq(dev_APOORVA->irq,irqdevid);
		//free_irq(IRQL,irqdevid);  dev_APOORVA->irq     //For SMP
		release_region(Start1,Len1);
		release_region(PLX_IO,Len2);
		return -1;
	}
/*for(ret =1;ret<=0x5f;ret+=2)
{
printk("\tIrq Allocation line is %d,%x  \n",readb(Base0+ret),ret);
printk("\tIrq Allocation pin  is %d %x  \n",readb(Base0+(ret+1)),ret+1);
}*/
	return 0;
}
...
static void __exit APOORVA_CARD1_EXIT(void)
{
	...
	//free_irq(dev_APOORVA->irq,irqdevid);
	//printk("\tIrq Allocation    %d ,%d\n",dev_APOORVA->irq,IntrLine);
	//free_irq(IRQL,irqdevid);	//For SMP
