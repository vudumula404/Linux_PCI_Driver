...
int main(void)
{
    ...
    int BcMod =1 ,rtModId = 3, mtModId = 2;
    char input[32];
    fd_set set;
    ...
    while (1)
    {
        ...
        switch (testNum)
        {
          ...
          case 8: // BC MT Mode                    
                ...
                for(i=0; i < 128; i++)
                  {
                      MemWrite(1,(0x20280+i),0xffff,1); ///changed add 20280 29/01/26
                  }
                MemWrite(1,(0x20102),0x1000,1);
                MemWrite(1,(0x20103),0x4000,1);
                  RegWrite(1,0x7,0x8000,1);
                RegWrite(1,0x1,0xD54f,1);
                RegWrite(1,0x2,0x0,1);
              
                 RegWrite(1,0x8,0x0,1);
                
                pthread_create(&mtThread, NULL, StartMTThread, &BcMod);
               // RegWrite(1,0x1,0xD44f,1);
                // (void)DefMode(1, 0xD44f);
                //RegWrite(1,0x7,0x8000,1);
                BCSchedule_NEW(NoOfTimes, 1,channel);

                printf("Reg add=%08x  Reg data=%04x\n",0x1,RegRead(1,0x1,1));
                printf("Reg add=%08x  Reg data=%04x\n",0x7,RegRead(1,0x7,1));
                getchar();
                RegWrite(1,0x3,0x1,1);
...
void* StartMTThread(void* arg)
{
    ...
    last =MemRead(MTModId, 0x20102, 1);// ReadMsgMtCounter(MTModId);
    printf("last =%04x %d\n",last,MTModId);
 ...
 void BCSchedule_NEW(int Times, unsigned char module_id, unsigned char channel)
{
  ...
  {    
	...
	DefMajorFrame(module_id, 0, 1, major_frames);
        ...
        DefFrame(module_id, 41, msg_sequence);          
        RunBc(module_id, 0, 1, 41, Times);
           printf("\tBC Scheduling Started Wait........! \n");*/
        getchar();
        printf("Reg add=%08x  Reg data=%04x\n",0x1,RegRead(module_id,0x1,1));
        printf("Reg add=%08x  Reg data=%04x\n",0x7,RegRead(module_id,0x7,1)); 
