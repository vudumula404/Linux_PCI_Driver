			...
			if (wMinorFrameIndex == MINOR_CYCLE_ZERO)
			{
				...
				if(Mc1LruInitialised==TRUE)
				{
					...
					for(int wMsgIndex = 0; wMsgIndex < uwMessCount1; wMsgIndex++)
					{
						...
						uhwFrameItems[wMsgIndex] = uhwMsgIndex1;
							...
							if(m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr<31)
							{
								EnableRtChannel_A(1,m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr,YES,CARD1);
								printf("Mod1RtAddress:%d SubAddress %d \n",m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr,m_stMsg[uhwMsgId].MsgDef.cmd_1.subaddr);
							}
				...
				if(Mc2LruInitialised==TRUE)
				{
					...
					//	printf("Mod2RtAddress:%d SubAddress %d \n",bc_page.cmd_1.taddr,bc_page.cmd_1.subaddr);
						...
						//BCRetry_A(1,AA,CARD1);
						DefMsg_A(2,BCSTACK_A,uhwMsgIndex2,bc_page,m_stMsg[uhwMsgId].databuffer,ubyWordCount ,CARD1);
						uhwFrameItems[wMsgIndex] = uhwMsgIndex2;

							if(m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr<31)
							{
								EnableRtChannel_A(2,m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr,YES,CARD1);
								printf("Mod2RtAddress:%d SubAddress %d \n",m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr,m_stMsg[uhwMsgId].MsgDef.cmd_1.subaddr);
							}
				...
				if(Mc3LruInitialised==TRUE)
				{
					...
					for(int wMsgIndex = 0; wMsgIndex < uwMessCount3; wMsgIndex++)
					{
						...
						//BCRetry_A(1,AA,CARD1);
						DefMsg_A(3,BCSTACK_A,uhwMsgIndex3,bc_page,m_stMsg[uhwMsgId].databuffer,ubyWordCount ,CARD1);
						uhwFrameItems[wMsgIndex] = uhwMsgIndex3;

							if(m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr<31)
							{
								EnableRtChannel_A(3,m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr,YES,CARD1);
				...
				/*for(int MsgNumber=0;MsgNumber<uhwMsgIndex1;MsgNumber++)
				{
					...
					if(m_stMsg[MsgNumber].MsgDef.cmd_1.taddr<31)
						EnableRtChannel_A(1,m_stMsg[MsgNumber].MsgDef.cmd_1.taddr,YES,CARD1);
				}*/
