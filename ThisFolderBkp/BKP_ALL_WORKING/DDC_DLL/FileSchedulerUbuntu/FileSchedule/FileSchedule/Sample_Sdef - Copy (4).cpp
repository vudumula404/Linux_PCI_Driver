			...
			if (wMinorFrameIndex == MINOR_CYCLE_ZERO)
			{
				...
				if(BcMod==1)
				{
					...
					for(int wMsgIndex = 0; wMsgIndex < uwMessCount1; wMsgIndex++)
					{
						...
						m_stMsg[uhwMsgId].MsgDef.cmd_1.taddr;
						bc_page.cmd_1.taddr;
	...
	while (!feof(fpstConfigFile))
	{
		...
		if(iRet == 3)
		{
			...
			if(ubySourceAddress<31)//Tx msg adrres
			{
				ActivateRT();
				EnableRtChannel_A(1,ubySourceAddress,YES,CARD1);
				printf("Mod1SourceRtAddress:%d Configured\n",ubySourceAddress);
			}
			if(ubyDestAddress<31)//Rx Msg addres
			{
				ActivateRT();
				EnableRtChannel_A(1,ubyDestAddress,YES,CARD1);
				printf("Mod1DestRtAddress:%d Configured\n",ubyDestAddress);
			}		
			m_ubyMessageNumber++;	
	...
	while (!feof(fpstConfigFile))
	{
		...
		if(iRet == 3)
		{
			...
			if(ubySourceAddress<31)//Tx msg adrres
			{
				ActivateRT2();
				EnableRtChannel_A(2,ubySourceAddress,YES,CARD1);
				printf("Mod2SourceRtAddress:%d Configured\n",ubySourceAddress);
			}
			if(ubyDestAddress<31)//Rx Msg addres
			{
				ActivateRT2();
				EnableRtChannel_A(2,ubyDestAddress,YES,CARD1);
				printf("Mod2DestRtAddress:%d Configured\n",ubyDestAddress);
			}		
			m_ubyMessageNumber++;	
	...
	while (!feof(fpstConfigFile))
	{
		...
		if(iRet == 3)
		{
			...
			if(ubySourceAddress<31)//Tx msg adrres
			{
				ActivateRT3();
				EnableRtChannel_A(3,ubySourceAddress,YES,CARD1);
				printf("Mod3SourceRtAddress:%d Configured\n",ubySourceAddress);				
			}
			if(ubyDestAddress<31)//Rx Msg addres
			{
				ActivateRT3();
				EnableRtChannel_A(3,ubyDestAddress,YES,CARD1);
				printf("Mod3DestRtAddress:%d Configured\n",ubyDestAddress);
			}		
			m_ubyMessageNumber++;	
