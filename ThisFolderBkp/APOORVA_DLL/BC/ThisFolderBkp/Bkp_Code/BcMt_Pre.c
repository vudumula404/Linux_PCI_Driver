...
if (def.cmd_1.t_r == RECEIVE_DDC && def.rt_rt == NO && def.mode_code == NO)
	...
	MemWrite(ModId,cmd_offset,cmd,1);
      //  printf("Address %x  CommandWord %x \n",cmd_offset,MemRead(ModId,cmd_offset,1));
	if(def.rt_rt == YES )
	{
	 ...
	 //printf("Address %x ControlWord %x \n",cntrl_offset,MemRead(ModId,cntrl_offset,1));
...
{	
  ...
  for (m=0; m<no_of_minor_frames; m++)
    ...
    if (number_of_messages <= 1024 && number_of_messages >= 1)	
    {
      ...
      MemWrite(Bus,(msg_offset+0x1000)+4,0xff,1);
      if (number_of_messages >1024 || number_of_messages < 1)
       return(-24);
    }
    else
			      ...
			      count=MemRead(Bus,MESS_COUNTER_DDC,1);
		                    ...
		                    //printf("MESS_COUNTER_DDC %d Readcount %x  temp %x num_of_messages  %d\n",MESS_COUNTER_DDC,count,temp,num_of_messages);
                 		    temp=0;
               		           ...
               		           //printf("Major Frame Completed %d\n",i);
               		    }
             		  }
...
void BCRetry1553_DDC(UCHAR ModId,USHORT sel)
{
	...
	if(sel)
	{
		...
		RegWrite(ModId,CONFIG_REG_DDC,(temp&0x0fe7)|(0x0013),1);
...
int main()
{
	...
	USHORT count;
       ...
       int m,init_pos,position,temp1;	
       unsigned int CmdStack=0x1000;
       USHORT num_of_messages=0,Mcnt=0,TimesValue;
	i = InitCard(1);
	if(i)
	...
	for (m=0; m<MinorCyleCnt; m++)
	      num_of_messages = num_of_messages + Min_Cyc[m].Mess_Count ;
	 for (i=0;i<Times;i++)
          {
             RegWrite(ModId,0x3,0x8000,1);    
          
            MemWrite(ModId,CmdStack,1,1);
            temp1 = 0xffff - (num_of_messages/**Times*/);
            //temp1 = 1000;

            MemWrite(ModId,MESS_COUNTER_DDC,temp1,1);//MESS_COUNTER_DDC 0x101
            
             MemWrite(ModId,TMP_CNTR_DDC,temp1,1);     
	    MemWrite(ModId,TMP_PTR_DDC,0,1);   
            StartReset_DDC(ModId,START_DDC);        
            count=MemRead(ModId,MESS_COUNTER_DDC,1);         
            printf("MESS_COUNTER_DDC %x Num_of_messagesFree  %d num_of_messages*Times %d\n",MESS_COUNTER_DDC,count,num_of_messages*Times);
            while (count != 0xFFFF)
            {
            count=MemRead(ModId,MESS_COUNTER_DDC,1);
            //printf("MESS_COUNTER_DDC %d Readcount %x  temp1 %x num_of_messages  %d\n",MESS_COUNTER_DDC,count,temp1,num_of_messages);
            temp1=0;
            }
             printf("Major Frame Completed\n");
          }
	/*StartReset_DDC(ModId,START_DDC);
	...
	/*printf("Data %x Addres %x\n",RegRead(ModId,0x00,1),0x00);
	printf("Data %x Addres %x\n",RegRead(ModId,0x01,1),0x01);
	printf("Data %x Addres %x\n",RegRead(ModId,0x02,1),0x02);
	printf("Data %x Addres %x\n",RegRead(ModId,0x03,1),0x03);
	printf("Data %x Addres %x\n",RegRead(ModId,0x07,1),0x07);
	printf("Data %x Addres %x\n",RegRead(ModId,0x08,1),0x08);
	printf("Data %x Addres %x\n",RegRead(ModId,0x09,1),0x09);
	printf("Data %x Addres %x\n",RegRead(ModId,0x10,1),0x10);
	printf("Data %x Addres %x\n",RegRead(ModId,0x24,1),0x024);*/
