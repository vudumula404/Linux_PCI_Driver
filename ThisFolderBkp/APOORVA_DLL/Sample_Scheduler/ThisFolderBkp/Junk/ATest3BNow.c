...
short DefMsg1553(UCHAR ModId,unsigned short msg_num,MSG_DEF def,unsigned short data_buffer[],unsigned short num_of_data_words,unsigned short IntMsgGap)
{
  ...
  if (def.cmd_1.t_r == RECEIVE && def.rt_rt == NO && def.mode_code == NO)
	...
	MemWrite(ModId,cmd_offset,cmd,1);
        //printf("msg_num %d Address [Hex:%x][Dec:%d] CommandWord %x ModId %d\n",msg_num,cmd_offset,cmd_offset,MemRead(ModId,cmd_offset,1),ModId);
               ...
               if (lastDesc == 0x2000)   // wrap-around
                ...
                // if (prevDesc != 0)
                  //{
                     ...
                     fprintf(logFile, "currentDesc=%04X  prev=%04X last=%04X (diff=%d) msgCount %d\n",
                                  currentDesc,prevDesc, lastDesc, diff,msgCount);
                 /*if (msgCount % 1000 == 0)
...
void* MTThread_Poll(void* arg)
{
    ...
    unsigned short ptr = 0, temp;
    unsigned short cmd = 0, rtAddr = 0, trBit = 0, subAddr = 0, wc_or_mode = 0;
    unsigned short currentDesc = 0, lastDesc = 0, prevDesc = 0;
    unsigned long msgCount = 0;
    int diff = 0;
    struct timeval tv;
    struct tm* t;
    ...
    while (1)
    {
        currentDesc = RegRead(MTModId, 0xB, 1); // hardware descriptor pointer

        if (currentDesc != 0)
        {
            if (lastDesc != currentDesc)
            {
                if (lastDesc == 0x2000)  // wrap-around condition
                    lastDesc = 0x1004;
                lastDesc = currentDesc;
                diff = lastDesc - prevDesc;

                // log descriptor sequence check
                if (diff != 4 && prevDesc != 0)
                {
                ...
                {
                    // In RT-to-RT, first two words in data block are:
                    // [0] = 2nd Command Word, [1] = Transmit RT Status
                    ...
                    fprintf(logFile, "RT-to-RT Detected: 2ndCmd=0x%04X TxRT=%u SA=%u WC=%u TxStatus=0x%04X\n",
                            secondCmd, rt_tx, sa_tx, wc_tx, txStatus);

                    // Skip 2 words for actual data start
                    ptr += 2;
                    temp = wc_tx;   // actual data words count
                }
               
                temp = buffer.RecCmdWrd.bitwise.WrdCntOrMde;
                if (temp == 0) temp = 32;

                // Adjust pointer for Transmit direction
                if (buffer.RecCmdWrd.bitwise.TrnRecv == 1)
                    ptr++;
                ...
                t = localtime(&tv.tv_sec);
                fprintf(logFile,
                        "\nTime [%02d:%02d:%02d.%03ld] Msg#%lu Desc=%04X\n",
                        t->tm_hour, t->tm_min, t->tm_sec,
                        tv.tv_usec / 1000, msgCount, lastDesc);

                // Log header
                fprintf(logFile,
                        "BlkSts=0x%04X  TimeTag=0x%04X  Cmd=0x%04X\n",
                ...
                char dir = (trBit == 1) ? 'T' : 'R';
                fprintf(logFile, "Decoded: RT=%u Sub=%u Dir=%c WC/Mode=%u\n",
                        rtAddr, subAddr, dir, wc_or_mode);

                if (subAddr == 0) wc_or_mode = 1; // Mode command fix
                ...
                for (int i = 0; i < wc_or_mode; i++)
                {
                    buffer.DataPack[i] = MemRead(MTModId, ptr++, 1);
                    len += snprintf(line + len, sizeof(line) - len, "%04X ", buffer.DataPack[i]);
                    if ((i + 1) % 8 == 0) len += snprintf(line + len, sizeof(line) - len, "\n");
                }
                fprintf(logFile, "%s\n", line);
            ...
            {
                // 🧩 tiny pause to reduce CPU stress but not delay polling
    #if defined(__x86_64__) || defined(__i386__)
                _mm_pause();    // Intel/AMD - efficient short pause (~10-50ns)
    #else
                asm volatile("yield");  // ARM equivalent (no-op if unsupported)
    #endif
            }
        }
    }
        ...
        if (currentDesc != 0)
        {
            if (lastDesc != currentDesc)
            {
                ...
                if (buffer.BlockSts.value & 0x0800)
                {
                    ...
                    fprintf(logFile,
                            "Decoded: RT=%u Sub=%u Dir=X (RT-to-RT) WC=%u  2ndCmd=0x%04X TxRT=%u SA=%u TxStatus=0x%04X\n",
    ...
    while (1)
    {
        ...
        if (currentDesc != 0)
        {
            if (lastDesc != currentDesc)
            {
                ...
                t = localtime(&tv.tv_sec);
                fprintf(logFile,
                        "\nTime [%02d:%02d:%02d.%06ld] Msg#%lu Desc=%04X\n",
                        t->tm_hour, t->tm_min, t->tm_sec,
                        tv.tv_usec, msgCount, lastDesc);
                /* fprintf(logFile,
                        "\nTime [%02d:%02d:%02d.%06ld] Msg#%lu Desc=%04X  Δt=%.3f ms\n",
                        t->tm_hour, t->tm_min, t->tm_sec,
                        tv.tv_usec, msgCount, lastDesc, delta_ms);*/
...
void BCSchedule_Bus()
{
	...
	DefMajorFrame(1, 0, 8, major_frames);
        ...
        RunBcMajorFrame(1, 0, 8, major_frames, 20);
        getchar();
