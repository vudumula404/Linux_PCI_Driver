...
void* MTThread_Poll(void* arg)
{
    ...
    MNTPACK buffer;
    unsigned short ptr = 0, temp,msgNo=0;
    unsigned short cmd = 0, rtAddr = 0, trBit = 0, subAddr = 0, wc_or_mode = 0;
    ...
    printf("wait......Enter To Read!\n");
    getchar();
    getchar();
    for(msgNo=1;msgNo<=1000;msgNo++)
    ReadMonPacket1553(MTModId, msgNo, &buffer,msgNo);
    exit(1);
    ...
    while (1)
    {
        ...
        if (currentDesc != 0)
        {
            ...
            {
              //printf("msgCount %d Reg(0xB)=%04X\n", msgCount,RegRead(MTModId, 0xB, 1));
    ...
    while (1)
    {
        ...
        if (currentDesc != 0)
        {
            ...
            {         
                ...
                t = localtime(&tv.tv_sec);
                fprintf(logFile,
                        "\nTime [%02d:%02d:%02d.%06ld] Msg#%lu Desc=%04X currentDesc=%04X pos=%d Offset=%04X \n",
                        t->tm_hour, t->tm_min, t->tm_sec,
                        tv.tv_usec, msgCount, lastDesc,currentDesc,pos,Offset);
                printf("\nTime [%02d:%02d:%02d.%06ld] Msg#%lu Desc=%04X currentDesc=%04X pos=%d Offset=%04X \n",
                        t->tm_hour, t->tm_min, t->tm_sec,
                        tv.tv_usec, msgCount, lastDesc,currentDesc,pos,Offset);
                /* fprintf(logFile,
                   ...
                   //if ((buffer.BlockSts.value & 0x0200)==0)
                   {
                      ...
                      for (int i = 0; i < temp; i++)
                      {
                        buffer.DataPack[i] = MemRead(MTModId, ptr++, 1);
                          ...
                          /*if(buffer.DataPack[i]!=0)
                               fprintf(logFile,"MsgN %d RxErr:[%d] Data[%04X]\n",lastDesc, i,buffer.DataPack[i]);
                          else*/
                            fprintf(logFile,"%04X ", buffer.DataPack[i]);
                        if ((i + 1) % 8 == 0)
                          fprintf(logFile, "\n");                       
