...
void SelMsgMt1553(UCHAR ModId)
{
	...
	MemWrite(ModId,0x102,0x1000,1);				//CMD word Stk Ptr//0x400
	MemWrite(ModId,0x103,0x4000,1);				//Data word Stk Ptr//0x800
	/*MemWrite(ModId,0x102,0x400,1);				//CMD word Stk Ptr//0x400
	MemWrite(ModId,0x103,0x800,1);				//Data word Stk Ptr//0x800*/
	
	printf(" 0x102 -> %x 0x103 %x\n",MemRead(ModId, 0x102,1),MemRead(ModId, 0x103,1));
...
void ReadMonPacket1553(UCHAR ModId, unsigned short DespNo, MNTPACK *MontDataBuff,unsigned long Lcount)
{
   ...
   // unsigned short Offset = 0x400 + ((DespNo * 4) - 4);
    ...
    //if(DespNo>126)
    {
      printf("DespNo %d MontDataBuff->DataBlkPtr %x CmdStack %x %d\n",DespNo,MontDataBuff->DataBlkPtr,MemRead(ModId, 0x102,1),Lcount);
      //printf("Txptr %x\n",);
     }    
...
void ReadMonPacket1553(UCHAR ModId, unsigned short DespNo, MNTPACK *MontDataBuff,unsigned long Lcount)
{
    ...
    // --- Calculate CmdStack offset for this DespNo ---
    unsigned short CmdStackOffset = 0x1000 + ((DespNo * 4)-4);
    //unsigned short CmdStackVal    = MemRead(ModId, /*CmdStackOffset*/0x102, 1);//MemRead(ModId, /*CmdStackOffset*/0x102, 1);
    fprintf(logFile,"DespNo %d MontDataBuff->DataBlkPtr %04X TimeTag %04X CmdWord %x BSW %x\n",DespNo, MontDataBuff->DataBlkPtr, MontDataBuff->TimeTag,MontDataBuff->RecCmdWrd.value,MontDataBuff->BlockSts.value );
    printf("DespNo %d MontDataBuff->DataBlkPtr %04X TimeTag %04X CmdWord %x BSW %x\n",DespNo, MontDataBuff->DataBlkPtr, MontDataBuff->TimeTag,MontDataBuff->RecCmdWrd.value,MontDataBuff->BlockSts.value );
    /*printf("DespNo %d MontDataBuff->DataBlkPtr %04X CmdStack %04X %lu BSW %x\n",DespNo, MontDataBuff->DataBlkPtr, CmdStackVal, Lcount,MemRead(ModId, Offset,     1));*/
     /*printf("DespNo %d MontDataBuff->DataBlkPtr %04X CmdStack %04X %lu BSW %x\n",DespNo, MemRead(ModId,0x103, 1), CmdStackVal, Lcount,MemRead(ModId, Offset,     1));*/
    /*for (int i = 0; i < 15; i++) 
    {
       printf("DespNo %d MontDataBuff->DataBlkPtr MontDataBuff->DataPack[i] = RegRead(ModId, i, 1);
    }*/
   // CmdStackOffset = CmdStackVal;
    // --- Verify CmdStack increments properly ---
    /*static unsigned short prevCmdStack = 0;

    if (DespNo == 1) {
        // First descriptor → reset baseline
        prevCmdStack = CmdStackOffset;
    }
    else
    {
      unsigned short expected = prevCmdStack + 4;        
      // Optional: handle rollover (e.g., 0x1FFC → 0x1000)
      if (expected > 0x1FFC) expected = 0x1000;

      if (CmdStackOffset != expected) {        
              printf("ERROR: CmdStack not incremented properly at DespNo %d "
              "(got %04X, expected %04X \t%lu)\n",
              DespNo, CmdStackOffset, expected, Lcount);
      }
      prevCmdStack = CmdStackOffset;
    }*/

    // --- Debug print ---
  //  printf("DespNo %d MontDataBuff->DataBlkPtr %04X CmdStack %04X %lu\n",DespNo, MontDataBuff->DataBlkPtr, CmdStackVal, Lcount);

    // --- Decode word count ---
    temp = MontDataBuff->RecCmdWrd.bitwise.WrdCntOrMde;
    if (temp == 0) temp = 32;

    // --- Adjust pointer for transmit ---
  /*  if (MontDataBuff->RecCmdWrd.bitwise.TrnRecv == 1) {
        ptr++;
    }*/
    ...
    t = localtime(&tv.tv_sec);
    fprintf(logFile, "\nTime Stamp [%02d:%02d:%02d.%03ld.%03ld] Packet #%lu\n",
            t->tm_hour, t->tm_min, t->tm_sec,
            tv.tv_usec / 1000,
            tv.tv_usec % 1000,
            Lcount);
...
void ReadMonPacket1553(UCHAR ModId, unsigned short DespNo, MNTPACK *MontDataBuff,unsigned long Lcount)
{
   ...
   // printf("MsgCount %d MontDataBuff->DataBlkPtr %04X TimeTag %04X CmdWord %x BSW %x\n",Lcount, MontDataBuff->DataBlkPtr, MontDataBuff->TimeTag,MontDataBuff->RecCmdWrd.value,MontDataBuff->BlockSts.value );
    /*printf("DespNo %d MontDataBuff->DataBlkPtr %04X CmdStack %04X %lu BSW %x\n",DespNo, MontDataBuff->DataBlkPtr, CmdStackVal, Lcount,MemRead(ModId, Offset,     1));*/
     /*printf("DespNo %d MontDataBuff->DataBlkPtr %04X CmdStack %04X %lu BSW %x\n",DespNo, MemRead(ModId,0x103, 1), CmdStackVal, Lcount,MemRead(ModId, Offset,     1));*/
...
/*
    for (int i = 0; i < wc_or_mode; i++) {
        ...
        printf("%04X ", MontDataBuff->DataPack[i]);
        if ((i + 1) % 8 == 0) printf("\n");
    }*/
...
int MtMsgCnt(unsigned char MTModId) {
    ...
    return ((MemRead(MTModId, 0x102, 1)-0x1000)/4);//((CMDDesc - 0x1000) / 4);
}
#if 1
void* MTThread_Poll(void* arg) 
{
    ...
    MNTPACK buffer;
    unsigned short Offset = 0;//0x1000 + ((DespNo * 4) - 4);
    unsigned short ptr=0, temp;
    unsigned short cmd=0, rtAddr=0, trBit=0, subAddr=0, wc_or_mode=0;
    unsigned long msgCount = 0; 
    unsigned short pos = 1;
    unsigned short currentDesc = 0, lastDesc = 0;
    unsigned short descAddr;
    unsigned short prevDesc = 0;
    int diff=0;
    ...
    RegWrite(MTModId, 0x3, 0x1, 1);
    lastDesc = RegRead(MTModId, 0xB, 1);
    ...
    {
        currentDesc = RegRead(MTModId, 0xB, 1);  // hardware pointer
        //currentDesc = MemRead(MTModId, 0x102, 1);
        // usleep(1);
        //printf("Monitor thread running... currentDesc=%04X\n", currentDesc);
        if(currentDesc!=0)
        {
          while (lastDesc != currentDesc) 
          {        
              //Offset = 0x1000 + ((pos * 4) - 4);
             // descAddr = lastDesc;
               if (lastDesc == 0x2000)   // wrap-around
                  lastDesc = 0x1004;
                  lastDesc = currentDesc;  
                  
                   // ---- check for abnormal increment ----
                // if (prevDesc != 0)
                  //{
                      diff = lastDesc - prevDesc;
                      if (diff != 4)
                      {
                          //printf("Warning: Descriptor jump! Message Count: %lu prev=%04X last=%04X (diff=%d)\n",msgCount,prevDesc, lastDesc, diff);
                          fprintf(logFile, "Warning: Descriptor jump! prev=%04X last=%04X currentDesc=%04X (diff=%d)\n",
                                  prevDesc, lastDesc,currentDesc, diff);
                      }
                     fprintf(logFile, "currentDesc=%04X  prev=%04X last=%04X (diff=%d)\n",
                                  currentDesc,prevDesc, lastDesc, diff);
                  // prevDesc = lastDesc;
                  if (diff > 1)
                  //else
                  {
                    buffer.BlockSts.value   = MemRead(MTModId, (lastDesc-4),     1);
                    buffer.TimeTag          = MemRead(MTModId, (lastDesc-4) + 1, 1);
                    buffer.DataBlkPtr       = MemRead(MTModId, (lastDesc-4) + 2, 1);
                    ptr                     = buffer.DataBlkPtr;
                    buffer.RecCmdWrd.value  = MemRead(MTModId, (lastDesc-4) + 3, 1);
                   if (msgCount % 1000 == 0)
                    printf("Message Count: %lu lastDesc %04x currentDesc %04x\n", msgCount,lastDesc,currentDesc);
                    #if 1
                    ...
                    char line[512];  // large enough for up to 32 words * 5 chars each
                    int len = 0;
                    ...
                    /*for (int i = 0; i < wc_or_mode; i++) {
                        ...
                        len += snprintf(line + len, sizeof(line) - len, "%04X ", buffer.DataPack[i]);
                ...
                //ReadMonPacket1553(MTModId, pos, &buffer,msgCount); 
                msgCount++;
        ...
        {        
           ...
           // descAddr = lastDesc;
             if (lastDesc == 0x2000)   // wrap-around
                ...
                buffer.BlockSts.value   = MemRead(MTModId, lastDesc,     1);
                buffer.TimeTag          = MemRead(MTModId, lastDesc + 1, 1);
                buffer.DataBlkPtr       = MemRead(MTModId, lastDesc + 2, 1);
                ptr                     = buffer.DataBlkPtr;
                buffer.RecCmdWrd.value  = MemRead(MTModId, lastDesc + 3, 1);
                temp = buffer.RecCmdWrd.bitwise.WrdCntOrMde;
                if (temp == 0) temp = 32;
                // --- Timestamp logging ---
                struct timeval tv;
    ...
    while (ThreadRunFlag == 0) 
    {
        ...
        {        
           ...
           // descAddr = lastDesc;
            ...
            msgCount++;
          //if(buffer.BlockSts.value!=0x8100)
          {
                   printf("Msg %lu | Desc %04X | CurDesc %04X | TimeTag %04X | CmdWord %04X | BSW %04X\n",
                   msgCount,
                   lastDesc,
                   currentDesc,
                   buffer.TimeTag,
            ...
            // Advance descriptor step by step
            //lastDesc += 4;
        }

        // Sleep a little if no new messages
       // usleep(1000);
    }
...
void* MTThread_Poll(void* arg) 
{
   unsigned char MTModId = *((unsigned char*)arg);
    ...
    unsigned short cmd=0,rtAddr=0,trBit=0,subAddr=0,wc_or_mode=0,count=0;
   // unsigned short cmd=0, rtAddr=0, trBit=0, subAddr=0, wc_or_mode=0;
    unsigned short Offset = 0;//0x1000 + ((DespNo * 4) - 4);
    unsigned short ptr=0, temp;
    struct timespec ts;
    ...
    lastDesc = RegRead(MTModId, 0xb,1);//(((RegRead(MTModId, 0xb,1))-0x1000)/4);//MtMsgCnt(MTModId);
    //lastDesc=0;
    //printf("Wait RegRead 0xb %x\n",RegRead(MTModId, 0xb,1));
    printf("Wait Mt Running\n");
    /*getchar();
    getchar();
    for(count=1;count<1025;count++)
      //printf("Wait RegRead 0xb %x\n",RegRead(MTModId, 0xb,1));
      ReadMonPacket1553(MTModId, count, &buffer,Lcount);   
      exit(1);*/
    while (ThreadRunFlag == 0) 
    {      
       ...
       // if (pthread_mutex_trylock(&mutexObj) == 0)         
        {
          currentDesc = RegRead(MTModId, 0xb,1);//(((RegRead(MTModId, 0xb,1))-0x1000)/4);//MtMsgCnt(MTModId);
          /*ts.tv_sec = 0;            // seconds
          ts.tv_nsec = 0;//5 * 1000000; // 5 ms = 5,000,000 ns
          nanosleep(&ts, NULL);*/
          //if (lastDesc != currentDesc) 
          {
            while (lastDesc != currentDesc ) 
            {
               /*if (lastDesc > 0x1FFC)   // wrap-around
                    lastDesc = 0x1000;*/
                ...
                msgCount++; // increment processed message count
                pos = (pos >= MSG_COUNT) ? 1 : pos + 1; 
                printf("Msg %lu | Desc %04X | currentDesc %04x| TimeTag %04X | CmdWord %04X | BSW %04X\n",
                       msgCount,
                       lastDesc,
                       currentDesc,
                       buffer.TimeTag,
               ...
               /* Offset = 0x1000 + ((pos * 4) - 4);
                ...
                fprintf(logFile,"MsgNo %d pos %d DataBlkPtr %04X TimeTag %04X CmdWord %x BSW %x\n",Lcount,pos, MemRead(MTModId, Offset + 2, 1), MemRead(MTModId, Offset + 1, 1),MemRead(MTModId, Offset + 3, 1),MemRead(MTModId, Offset,     1) );
                //printf("MsgNo %d pos %d DataBlkPtr %04X TimeTag %04X CmdWord %x BSW %x currentDesc %x lastDesc %x\n",Lcount,pos, MemRead(MTModId, Offset + 2, 1) , MemRead(MTModId, Offset + 1, 1),MemRead(MTModId, Offset + 3, 1),MemRead(MTModId, Offset,     1),currentDesc,lastDesc); 
                printf("MsgNo %d pos %d \r",Lcount,pos);   
                Lcount++;             
                pos = (pos >= MSG_COUNT) ? 1 : pos + 1; */  
...
void* MTThread_Poll(void* arg) 
{
    ...
    unsigned short currentDesc = 0, lastDesc = 0;
    unsigned short descAddr;
    MNTPACK buffer;   // local buffer to hold descriptor fields
    unsigned long msgCount = 0;  // total processed messages
    
    signal(SIGINT, handle_sigint); // Graceful Ctrl+C handling
    
    // Initial pointer from hardware
    lastDesc = MemRead(MTModId, 0x102, 1);

    printf("Monitor thread running...\n");

    while (ThreadRunFlag == 0) 
    {
        currentDesc = MemRead(MTModId, 0x102, 1);   // hardware pointer
        ...
        if (lastDesc != currentDesc) 
        {
            while (lastDesc != currentDesc)
            {
                ...
                buffer.BlockSts.value   = MemRead(MTModId, descAddr,     1);
                buffer.TimeTag          = MemRead(MTModId, descAddr + 1, 1);
                buffer.DataBlkPtr       = MemRead(MTModId, descAddr + 2, 1);
                buffer.RecCmdWrd.value  = MemRead(MTModId, descAddr + 3, 1);
                ...
                printf("Msg %lu | Desc %04X | currentDesc %04x| TimeTag %04X | CmdWord %04X | BSW %04X\n",
                 ...
                 fflush(stdout);
                lastDesc = currentDesc;
                //lastDesc += 4;  // move to next descriptor
            }
        }
        else
        {
            usleep(1000);   // avoid busy loop when no new data
        }
    }
...
void* MTThread_Poll(void* arg) 
{
    ...
    MNTPACK buffer;   
    static unsigned int success_count = 0, fail_count = 0;
    unsigned long msgCount = 0;  
    unsigned short pos = 1;
    signal(SIGINT, handle_sigint); // Graceful Ctrl+C handling
    ...
    while (ThreadRunFlag == 0) 
    {
        currentDesc = MemRead(MTModId, 0x102, 1);   // hardware pointer        
         usleep(1);
          if (msgCount > 2000) 
          {
            ThreadRunFlag = 1;   // force exit
            break;               // break inner loop
          }
        if (lastDesc != currentDesc) 
        {
          //  while (currentDesc != lastDesc)
            {
                msgCount++; 
                ...
                if (lastDesc > 0x1FFC)   // wrap-around
                    lastDesc = 0x1000;
              /* if(msgCount>=1024)
                MemWrite(MTModId, 0x102, 0x1000,1);*/
                 descAddr = lastDesc;
                buffer.BlockSts.value   = MemRead(MTModId, descAddr,     1);
                buffer.TimeTag          = MemRead(MTModId, descAddr + 1, 1);
                buffer.DataBlkPtr       = MemRead(MTModId, descAddr + 2, 1);
                buffer.RecCmdWrd.value  = MemRead(MTModId, descAddr + 3, 1);
                  ...
                  fprintf(logFile,"Msg %lu | LastDesc %04X | currentDesc %04X |  CmdWord %04X | BSW %04X TimeTag %04X \n",
                       msgCount,
                       lastDesc,
                       currentDesc,                       
                       buffer.RecCmdWrd.value,
                       buffer.BlockSts.value, buffer.TimeTag);     
                  printf("Msg %lu | LastDesc %04X | currentDesc %04X |  CmdWord %04X | BSW %04X TimeTag %04X \n",
                       msgCount,
                       lastDesc,
                       currentDesc,                       
                       buffer.RecCmdWrd.value,
                ...
                /*if(buffer.BlockSts.value!=0x8100)  
                {
                       printf("Msg %lu | Desc %04X | TimeTag %04X | CmdWord %04X | BSW %04X \n",
                       msgCount,
                       lastDesc,
                       buffer.TimeTag,
                       buffer.RecCmdWrd.value,
               ...
               /* if (buffer.BlockSts.value== 0x8100) {
                ...
                //printf("Completed: %u, Failed: %u\r", success_count, fail_count);               
               ...
               /* printf("Msg %lu | Desc %04X | TimeTag %04X | CmdWord %04X | BSW %04X\n",
                ...
                // --- Decode Cmd Word ---
                unsigned short cmd      = buffer.RecCmdWrd.value;
                unsigned short rtAddr   = (cmd >> 11) & 0x1F;
                unsigned short trBit    = (cmd >> 10) & 0x01;
                unsigned short subAddr  = (cmd >> 5)  & 0x1F;
                unsigned short wc_or_mode = cmd & 0x1F;
                if (wc_or_mode == 0) wc_or_mode = 32;
                ...
                if (subAddr == 0 || subAddr == 31) {
                    // Mode code with data (only some mode codes carry 1 word)
                    wc_or_mode = 1;
                }

                unsigned short ptr = buffer.DataBlkPtr;
                if (trBit == 1) {
                    ptr++;  // adjust for transmit messages
                }
