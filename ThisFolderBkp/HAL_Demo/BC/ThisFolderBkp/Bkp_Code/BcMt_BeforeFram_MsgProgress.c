...
void UpdateMsgData(UCHAR ModId, unsigned short msg_num, unsigned short *new_data, int num_words, int direction)
{
    ...
    if (direction == RECEIVE_DDC)   // BC → RT
        buff_offset = msg_base + 2;
    else if (direction == TRANSMIT_DDC) // RT → BC
...
/*void UpdateMsg1553(UCHAR ModId, 
                   ...
                   int direction)
{
    unsigned int CmdStack   = 0x1000;
    unsigned int DataStack  = 0x4000;
    unsigned int cmd_base   = CmdStack  + (msg_num - 1) * 4;
    unsigned int data_base  = DataStack + (msg_num - 1) * 40;
    unsigned short ctrl1;
    int i;

    //
    // 1. Update Command Words
    //
    if (cmd1 != 0xFFFF)  // use 0xFFFF if you don’t want to change
        MemWrite(ModId, cmd_base + 0, cmd1, 1);

    if (cmd2 != 0xFFFF) 
        MemWrite(ModId, cmd_base + 1, cmd2, 1);

    //
    // 2. Update Control Word safely
    //
    ctrl1 = MemRead(ModId, cmd_base + 2, 1);

    if (num_words > 0) {
        // clear old length (bits 8–12)
        ctrl1 &= ~0x1F00;
        ctrl1 |= ((num_words & 0x1F) << 8);
    }

    if (direction == RECEIVE_DDC || direction == TRANSMIT_DDC) {
        // clear old direction bit (10)
        ctrl1 &= ~0x0400;
        ctrl1 |= ((direction & 0x1) << 10);
    }

    MemWrite(ModId, cmd_base + 2, ctrl1, 1);

    //
    // 3. Update Data if provided
    //
    if (data != NULL && num_words > 0) {
        unsigned int data_offset;

        if (direction == RECEIVE_DDC)      // BC → RT
            data_offset = data_base + 2;
        else if (direction == TRANSMIT_DDC) // RT → BC
            data_offset = data_base + 4;
        else
            return;

        for (i = 0; i < num_words; i++)
            MemWrite(ModId, data_offset + i, data[i], 1);
    }
}*/
/*int UpdateMsgCmdData(UCHAR ModId,
                     ...
                     int direction)   // RECEIVE_DDC / TRANSMIT_DDC
{
    unsigned int DataStack = 0x4000;
    ...
    // update cmd2 (for RT–RT only)
    if (cmd2)
        MemWrite(ModId, base + 2, cmd2, 1);

    // decide offset for data based on direction
    if (direction == RECEIVE_DDC) {
        // BC → RT
        offset = base + 2;
    }
    else if (direction == TRANSMIT_DDC) {
        // RT → BC
        offset = base + 4;
    }
    else {
        // fallback (Mode Code or RT–RT)
        offset = base + 2;
    }
...
int UpdateMsgCmdData(UCHAR ModId,
                     ...
                     unsigned short *data_buffer,
                     unsigned short num_of_data_words,
                     int direction)   // RECEIVE_DDC / TRANSMIT_DDC / RT_RT / MODE_CODE
{
    unsigned int DataStack = 0x4000;
    unsigned int base      = DataStack + (msg_num - 1) * 40;
    ...
    switch (direction)
    {
        case RECEIVE_DDC:   // BC → RT
            offset = base + 2;
            for (i = 0; i < num_of_data_words; i++)
                MemWrite(ModId, offset + i, data_buffer[i], 1);
            break;

        case TRANSMIT_DDC:  // RT → BC
            offset = base + 4;
            for (i = 0; i < num_of_data_words; i++)
                MemWrite(ModId, offset + i, data_buffer[i], 1);
            break;
        ...
        case 3:   // Mode Code
          ...
          else if (tr == RECEIVE_DDC && (wcnt == 17 || wcnt == 20 || wcnt == 21)) {
              // RX special case
              if (num_of_data_words > 0)
	  ...
	  if(MsgFrame==1)
	  {
	    ...
	    RunBcMajorFrame1553(ModId,0,MinorCyleCnt,Min_Cyc,Times,1);
	     unsigned short new_data[32];
            ...
            UpdateMsgData(ModId, 96, new_data, 32, RECEIVE_DDC);
            
            for (i = 0; i < 32; i++)
                new_data[i] = 0xABBA+ i;   // some dynamic pattern
            UpdateMsgData(ModId, 97, new_data, 32, RECEIVE_DDC);
            for (i = 0; i < 32; i++)
                new_data[i] = 0xBABA + i;   // some dynamic pattern
            UpdateMsgData(ModId, 98, new_data, 32, RECEIVE_DDC);
            
            for (i = 0; i < 32; i++)
                new_data[i] = 0xCACA+ i;   // some dynamic pattern
            UpdateMsgData(ModId, 99, new_data, 32, RECEIVE_DDC);
