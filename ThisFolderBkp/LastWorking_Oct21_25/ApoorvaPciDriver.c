...
/*#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18))
	...
	if (request_irq(IntrLine, ISR, IRQF_SHARED, "APOORVA1553", irqdevid))
	{
          printk("APOORVA_PCI: Irq Allocation Failed for IRQ %d\n", IntrLine);
          return -EBUSY;
        } 
        else 
        {
          printk("APOORVA_PCI: IRQ %d registered successfully\n", IntrLine);
        }
