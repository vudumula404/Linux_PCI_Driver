...
typedef struct {
        unsigned int loop_tstfail : 1;
        unsigned int resp_timeout : 1;
        unsigned int msg_fmt_err  : 1;
        unsigned int stat_wrd_err : 1;
...
typedef struct {
        BSW_BITS_DDC  bsw_bits;               /*   block status bits       */
        STATUS_BITS_DDC  stat1_bits,stat2_bits ; /* status bits word 1 & 2   */
                CMD_DDC  cmd1_bits,cmd2_bits;   /* bits in the command word */
