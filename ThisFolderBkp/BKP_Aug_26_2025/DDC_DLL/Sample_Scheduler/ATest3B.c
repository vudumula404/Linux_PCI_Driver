...
MSG_DEF_DDC bc_page,bc_sync,bc_page_msg[32];
...
MNTPACK_DDC buffer1,buffer2,buffer3;
HANDLE hMutexObj;
...
void ReadMonPacket1553_DDC(UCHAR ModId, unsigned short DespNo, MNTPACK_DDC *MontDataBuff,unsigned long Lcount)
{
   ...
   // unsigned short Offset = 0x400 + ((DespNo * 4) - 4);
    ...
    //if(DespNo>126)
    {
      printf("DespNo %d MontDataBuff->DataBlkPtr %x CmdStack %x \n",DespNo,MontDataBuff->DataBlkPtr,MemRead(ModId, 0x102,1));
      //printf("Txptr %x\n",);
     }
     #if 0
    MontDataBuff->RecCmdWrd.value  = MemRead(ModId, Offset + 3, 1);
...
void ConfigureMt(unsigned char MTModId)
{
 ...
 RegWrite(MTModId, CONFIG_REG_DDC, 0x5000, 1);
  //SelMsgMt_DDC(MTModId);
  SelMsgMt(MTModId);
  //SelMsgMt(MTModId);
 /* RegWrite(MTModId, CONFIG_REG2_DDC, 0x8000, 1);
  RegWrite(MTModId, INTMASK_REG_DDC, 0x0800, 1);*/
       ...
       // if (pthread_mutex_trylock(&mutexObj) == 0) 
        {
          ...
          {
              ReadMonPacket1553_DDC(MTModId, pos, &buffer,Lcount);              
              Lcount++;             
...
//rtModId=3;
    ...
    for(char subaddress=1;subaddress<31;subaddress++)
    {
      ...
      WriteDataBlock_DDC(rtModId, subaddress, txBuffer);
    }
...
{
          int RetVal=0;
	  RetVal = ClearRam_DDC(ModuleId);	 
	 // printf("Retval of ClearRam is :%d ModId %d\n",RetVal,ModuleId);
	  ResetAce_DDC(ModuleId); 
	  RetVal = DefMode_DDC(ModuleId,BC_DDC);
...
void IntSyncT(UBIT16 syncval,UBIT16 ModuleId,UBIT16 CardId)
{
	...
	bc_sync.bus           = BUS_A;
	bc_sync.self_test     = NO_DDC;
	bc_sync.mask_bcst_bit = NO_DDC;
	bc_sync.broadcast     = YES_DDC;
	bc_sync.rt_rt         = NO_DDC; 
	bc_sync.mode_code     = YES_DDC;
	bc_sync.cmd_1.taddr   = 31;
	bc_sync.cmd_1.t_r     = RECEIVE_DDC;
	...
	{
		Pos1++;		
	        err =DefMsg_DDC(ModuleId,Pos1,bc_sync,data,17);
...
void IntBCbcst(UBIT16 no_of_mess,UBIT16 *sub_add,UBIT16 buffer[][32],UBIT16 *cnt,UBIT16 bus ,UBIT16 ModuleId,UBIT16 CardId)
{
	...
	{			
		bc_page_msg[i].bus           = BUS_A_DDC;
		bc_page_msg[i].self_test     = NO_DDC;
		bc_page_msg[i].mask_bcst_bit = NO_DDC; 
		bc_page_msg[i].broadcast     = YES_DDC;
		bc_page_msg[i].rt_rt         = NO_DDC; 
		bc_page_msg[i].mode_code     = NO_DDC;
	...
	if(ModuleId == MODULE_ID_0)
	{
		...
		{			
			...
			DefMsg_DDC(1,Pos1,bc_page_msg[i],(UBIT16 *)&buffer[i][0],cnt[i]);
			   ...
			   UBIT16 *dest_sub_addr,UBIT16 *source_sub_addr,UBIT16 *cnt,UBIT16 Bus ,UBIT16 ModuleId,UBIT16 CardId )
{
	...
	if(no_of_mess > 0)
	{
		for(i=0;i<no_of_mess;i++)
		{
			...
			{
				Pos1++;			
				err =DefMsg_DDC(1,Pos1,bc_page_msg[i],(UBIT16 *)&buffer[i][0],cnt[i]);
			}
		}
	}
}
...
UBIT16 IntBCsend(UBIT16 no_of_mess,UBIT16 *address,UBIT16 *sub_add,UBIT16 buffer[32][32],UBIT16 *cnt,UBIT16 Bus ,UBIT16 ModuleId,UBIT16 CardId)
{
	...
	for(i=0;i < no_of_mess;i++)
	{
		...
		if(ModuleId == MODULE_ID_0)
		{
			Pos1++;
			//SelectModule(MODULE_ID_0,CARD1);
			err =DefMsg_DDC(1,Pos1,bc_page_msg[i],/*(UBIT16 *)&buffer[i][0]*/data,cnt[i]);
		}
	}
	return(msg_return.bsw_bits.err_flag);
}
...
void B1553_Bus_Scheduling(void)
{
	...
	{		
		...
		DefFrame_DDC(1,Pos1,frame_items);
	}	
	if(Mc1LruInitialised == TRUE)
	{ 
		//SelectModule(0,CARD1);		
		RunBc_DDC(1,STACK_1_DDC,1,Pos1,1);
		//printf("RunBc..\n");
	}	
}
void Bus_Data_Recv(void)
...
UBIT16 IntBCrec_recv(UBIT16 buffer[32][32],UBIT16 MCyc,UBIT16 ModuleId,UBIT16 CardId )
{
	...
	if(ModuleId == MODULE_ID_0)
	{
		...
		{
			//SelectModule(MODULE_ID_0,CARD1);
			err=ReadMsgResult_DDC(STACK_1_DDC,j,&msg_return,CARD1);
