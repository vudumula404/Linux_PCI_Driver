...
RT_MSG_DDC RtBuff;
MNTPACK_DDC buffer;
unsigned short frame_items[128],data_buffer[40],temp_data;
   ...
   /* for (i = 0; i < temp; i++)
    {
        ...
        if (ptr == 0x1000) ptr = 0x800;
         ...
         // Logging
          ...
          fprintf(logFile, "\n[%02d:%02d:%02d] Packet #%d\n",
                  t->tm_hour, t->tm_min, t->tm_sec, Lcount);*/
          struct timeval tv;
          struct tm* t;
...
void* MTThread_Poll(void* arg) 
{
   ...
   //printf("Press Enter to display MT results... %d\n ",MTModId);       
       ...
       // if (pthread_mutex_trylock(&mutexObj) == 0) 
        {
          ...
          //printf("Press Enter to display MT results... %d currentDesc %d\n ",MTModId,currentDesc);   
        ...
        /*else 
        {
            // Mutex is locked by another thread; skip this cycle
            usleep(100);  // 100µs to prevent tight loop
        }*/
...
void ConfigureRT(UCHAR rtModId, unsigned char rtAddr)
{
    ...
    for (int i = 0; i < BUFFER_SIZE; ++i) txBuffer[i] = 0x3551+ i;
...
void Receive_Ofp_Data_MinorCycle7_bc(void)
{
	...
	{                
		...
		index1++;
	        ...
	        index1++;
		Bus1_address[index1]= 7 ; Bus1_sub_address[index1] = 6 ;Bus1_wordcnt[index1] = 282 ;
		index1++;
