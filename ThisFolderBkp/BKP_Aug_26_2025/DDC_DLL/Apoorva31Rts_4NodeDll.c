...
/*amar to be enable*/
#if 1
char SetRtAddrM_DDC(UCHAR ModId,unsigned short RtAddr,unsigned char CardId)
{
	unsigned short Rt;
	char Temp;
	Rt = RtAddr;
        RtAddr = CalculateParity_DDC(ModId,RtAddr);
        Temp = RtAddr;
...
void InitInterrupts_DDC(unsigned char Intr,unsigned char ModId)
{
    switch(ModId)
    {
        case 1:
...
void DefIntMask_DDC(UCHAR ModId, INT_DEF_DDC mask)
{
	...
	RegWrite(ModId,INTMASK_REG_DDC,value,1);
	//printf("MaskReg :%x, Value :%x",0x0,RegRead_DDC(INTMASK_REG_DDC));
}
...
int DefMajorFrame(UCHAR Bus,USHORT BCSTACK_A_B,USHORT no_of_minor_frames,MAJOR_FRAME frame_sequence[],UCHAR CardId)
{
...
unsigned long RunBcMajorFrame(UCHAR Bus,USHORT stackA_B,USHORT no_of_minor_frames,MAJOR_FRAME frame_sequence[],unsigned long times,UCHAR CardId)
{
	BOOL CardFlag = FALSE;
