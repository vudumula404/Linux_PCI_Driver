...
#include <dos.h>
#include "process.h"
#include "C:\FileSchedule\FileSchedule\Include\Libraries.h"
...
void fnExtractMessagesB3(char *);
void fnExtractMessagesB2(char *);
void RtfnExtractMessages(char *);
void RtfnExtractMessagesB3(char *);
void RtfnExtractMessagesB2(char *);
...
unsigned char fnGetMinorFrame(unsigned char ubyMessageNumber);
//void fnDefineSyncMsg();
unsigned char InterMsgGap(unsigned char ubyMessageNumber);
unsigned char fnGetFrequency(unsigned char ubyMessageNumber);
...
unsigned char m_ubyMessageNumber,m_ubyMessageNumber2,m_ubyMessageNumber3;
...
void fnExtractMessages(char * szFilename)
{
	...
	unsigned char	ubyTimeFrequency,ubyInterMsgGap,ubyReceiveMessageCount,ubyTransmitMessageCount;
	...
	while (!feof(fpstConfigFile))
	{
		...
		if(iRet == 3)
		{
			...
			ubyInterMsgGap		= InterMsgGap(m_ubyMessageNumber);
		...
		/*	if(ubySourceAddress<32)
			...
			*/
			m_stMsg[0][m_ubyMessageNumber].MsgDef.Int_Msg_Gap	 = ubyInterMsgGap;
...
void fnDefineSyncMsg()
{
	...
	//Define Sync Message for each minorframe on Channel A and Channel B
	 for( unsigned char ubyMinorFrameIndex = 0; ubyMinorFrameIndex < 8; ubyMinorFrameIndex++)
	 {
		...
		m_stMsg[0][m_ubyMessageNumber].MsgDef.mode_code     = YES;
		m_stMsg[0][m_ubyMessageNumber].MsgDef.rt_rt         = NO;
		m_stMsg[0][m_ubyMessageNumber].MsgDef.cmd_1.taddr   = 31;
		m_stMsg[0][m_ubyMessageNumber].MsgDef.cmd_1.t_r     = RECEIVE;
		m_stMsg[0][m_ubyMessageNumber].MsgDef.cmd_1.status  = 0xcccc;
		m_stMsg[0][m_ubyMessageNumber].MsgDef.loop          = 0x0000;
	...
	strcpy(m_stMsg[0][m_ubyMessageNumber].szMessageName, "PMC4A");
	for(int i=0; i<m_stMsg[0][m_ubyMessageNumber].MsgDef.cmd_1.wcnt; i++)
...
void fnDefineSyncMsg2()
{
	...
	strcpy(m_stMsg[1][m_ubyMessageNumber2].szMessageName, "PMC4A");
	for(int i=0; i<m_stMsg[1][m_ubyMessageNumber2].MsgDef.cmd_1.wcnt; i++)
...
void fnDefineSyncMsg3()
{
	...
	strcpy(m_stMsg[2][m_ubyMessageNumber3].szMessageName, "PMC4A");
	for(int i=0; i<m_stMsg[2][m_ubyMessageNumber3].MsgDef.cmd_1.wcnt; i++)
...
unsigned char InterMsgGap(unsigned char ubyMessageNumber)
{
	...
	sscanf(m_rgszMessagesDatabase[ubyMessageNumber],"%s%s%s%s%s%s%s%s%s%s",
				sFreq,sFreq,sFreq,sFreq,sFreq,sFreq,sFreq,sFreq,sFreq,sFreq);
				...
				if(Mc1LruInitialised==TRUE)
				{
					...
					for(int wMsgIndex = 0; wMsgIndex < uwMessCount1; wMsgIndex++)
					{
						...
						if (wMinorFrameIndex == MINOR_CYCLE_ZERO)
						{
							...
							if((bc_page.broadcast == TRUE) && (bc_page.cmd_1.taddr == 31)&& (bc_page.cmd_1.subaddr == 29)&&(bc_page.cmd_1.wcnt==2))//Update Timebroadcast data for MinorFrame 0
							{
