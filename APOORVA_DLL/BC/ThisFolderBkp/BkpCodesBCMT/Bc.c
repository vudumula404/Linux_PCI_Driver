...
if (def.cmd_1.t_r == RECEIVE_DDC && def.rt_rt == NO && def.mode_code == NO)
	...
	cmd_offset = 0x2000+((msg_num-1)*40)+1;
	...
	if(def.rt_rt == YES )
	{
		...
		else 
		{
			...
			cmd_offset= 0x2000+((msg_num-1)*40)+2;
			MemWrite(ModId,cmd_offset,cmd,1);
		}
	 }
	 ...
	 cntrl_offset = 0x2000+((msg_num-1)*40);
...
//	{
	...
	case 0 :
		...
		buff_offset = (0x2000 + (msg_num -1) * 40) + 2 ;
		for (i=0; i < num_of_data_words; i++)
		MemWrite(ModId,buff_offset+i,data_buffer[i],1);

		buff_offset = 0x2000 + ((msg_num-1)*40)+2+num_of_data_words;
		MemWrite(ModId,buff_offset,def.loop,1);

		buff_offset = 0x2000 + (msg_num - 1)*40+2+num_of_data_words+1;
		MemWrite(ModId,buff_offset,def.cmd_1.status,1);
		break;

	case 1 :

		buff_offset = (0x2000 + (msg_num -1)*40) + 4;
		for (i=0; i<num_of_data_words;i++)
		MemWrite(ModId,buff_offset+i,data_buffer[i],1);
		...
		buff_offset = 0x2000 + ((msg_num-1)*40)+3;
		MemWrite(ModId,buff_offset,def.cmd_1.status,1);
		break;
	...
	case 2 :
		...
		buff_offset = (0x2000 + (msg_num -1)*40) + 5;
		for (i=0; i<num_of_data_words;i++)
			MemWrite(ModId,buff_offset+i,data_buffer[i],1);

		buff_offset = 0x2000 + ((msg_num-1)*40)+5+num_of_data_words;
		MemWrite(ModId,buff_offset,def.cmd_1.status,1);
		break;
	...
	if (error == 0)
	 {
		for (i=0;i<times;i++)
		{
			MemWrite(ModId,0x800,init_pos,1);
...
int main()
{
	...
	if(i)
	 ...
	 //bc_page.retry_enable=YES;
	  ...
	  for(MinorCyle=0;MinorCyle<5;MinorCyle++)
	  {
	  /*Cnt_Time = frame_sequence[m].Mess_Count << 8;//<<9 APOORVA PCI
	    Cnt_Time = Cnt_Time | frame_sequence[m].MinorFrame_Time ;
	  ...
	  for(i=1;i<=Times;i++)
	  {
		  RunBc1553_DDC(ModId,STACK_1_DDC,1,MsgNo,1);/*issue start Command*/
		  //usleep(10);
	  }
