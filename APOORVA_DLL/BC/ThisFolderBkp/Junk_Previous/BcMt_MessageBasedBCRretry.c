...
void BCRetry1553_DDC(UCHAR ModId,USHORT sel)
{
	...
	if(sel)
	{
		...
		RegWrite(ModId,CONFIG_REG_DDC,RegRead(ModId,CONFIG_REG_DDC,1) | 0x0,1);//(0x8 Double Retry Enale 0 for Single retry)
		printf("BCRetry_DDC sel %d CONFIG_REG1_DDC %x\n",sel,RegRead(ModId,CONFIG_REG_DDC,1));
		switch(sel)
		{

		case 1:
		        RegWrite(ModId,CONFIG_REG_DDC,RegRead(ModId,CONFIG_REG_DDC,1) | 0x0,1);
			RegWrite(ModId,CONFIG_REG3_DDC,0x8000,1);
			val=RegRead(ModId,CONFIG_REG4_DDC,1);
			RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1200 ,1);//0x1300 alternate as//0x1380 aa//0x1200 same bus ss//1280 sa
		...
		case 2:
			...
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1300 ,1);//0x1300 alternate as//0x1380 aa//0x1200 same bus ss//1280 sa
			printf("CONFIG_REG3_DDC %x CONFIG_REG4_DDC %x\n",RegRead(ModId,CONFIG_REG3_DDC,1),RegRead(ModId,CONFIG_REG4_DDC,1));
			break;
		...
		/*case 3:
			...
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1280 ,1);//0x1300 alternate as//0x1380 aa//0x1200 same bus ss//1280 sa
			printf("CONFIG_REG3_DDC %x CONFIG_REG4_DDC %x\n",RegRead(ModId,CONFIG_REG3_DDC,1),RegRead(ModId,CONFIG_REG4_DDC,1));
			break;
		...
		case 4:
			...
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1380 ,1);//0x1300 alternate as//0x1380 aa//0x1200 same bus ss//1280 sa
			printf("CONFIG_REG3_DDC %x CONFIG_REG4_DDC %x\n",RegRead(ModId,CONFIG_REG3_DDC,1),RegRead(ModId,CONFIG_REG4_DDC,1));
			break;*/

		case 3:
		        RegWrite(ModId,CONFIG_REG_DDC,RegRead(ModId,CONFIG_REG_DDC,1) | 0x8,1);
			RegWrite(ModId,CONFIG_REG3_DDC,0x8000,1);
			val=RegRead(ModId,CONFIG_REG4_DDC,1);
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
			//RegWrite(ModId,CONFIG_REG4_DDC,val | 0x1180 ,1);
...
int main()
{
	...
	unsigned short		MemBuff,MsgNo=0,Data=0,Retry=0;
	...
	if(i)
	  ...
	  scanf("%x",&Data);*/
         ...
         /* printf("\n\t\tRetry Option : Same:1/Alter:2");
          printf("\n\t\tRetry Option : DoubleRetry Same:3/Alter:4::Same&Alter:5::Alter/Same:6\n\n");
	  scanf("%d",&Retry);*/
	...
	// RegWrite(ModId,0x23,0x5a5a,1);
	  ...
	  if(Ch	==	1)
	    ...
	    for(MsgNo=1;MsgNo<(MsgCnt+1);MsgNo++)
	    {
	      ...
	      //bc_page.cmd_1.t_r     = RECEIVE_DDC;//RECEIVE_DDC;	
	      bc_page.cmd_1.taddr   = ++RtAddre;
	      bc_page.cmd_1.t_r     = RECEIVE_DDC;
	     /*if((MsgNo>0)&&(MsgNo<300))
	  ...
	  //BCRetry1553_DDC(ModId,Retry);
	  for(POS1=0;POS1<(MsgCnt+1);POS1++)
