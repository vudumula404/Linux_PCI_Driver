...
void ReadMonPacket1553(UCHAR ModId, unsigned short DespNo, MNTPACK *MontDataBuff,unsigned long Lcount)
{
    ...
    unsigned short CmdStackVal    = MemRead(ModId, /*CmdStackOffset*/0x102, 1);
   ...
   // CmdStackOffset = CmdStackVal;
    ...
    else
    {
      ...
      if (CmdStackOffset != expected) {
          /*fprintf(logFile,
              "ERROR: CmdStack not incremented properly at DespNo %d "
       ...
       // if (pthread_mutex_trylock(&mutexObj) == 0) 
        {
          ...
          while (lastDesc != currentDesc) 
          {
              ...
              usleep(20);
              printf("lastDesc %d currentDesc %d 0x102 %x\n",lastDesc,currentDesc,MemRead(MTModId, 0x102, 1));
              Lcount++;             
