...
void main()
{
	...
	strcpy(szFilenameB1,"flm_b1.txt");
...
void fnExtractMessages(char * szFilename)
{
	...
	while (!feof(fpstConfigFile))
	{
		...
		{			
			...
			/*msgprep.MessageNumber[m_ubyMessageNumber-1]=m_ubyMessageNumber;
			msgprep.RxRtAddres[m_ubyMessageNumber-1] = ubyDestAddress;
			msgprep.RxRtSaddr[m_ubyMessageNumber-1] = ubyDestSubAddress;
			msgprep.TxRtAddres[m_ubyMessageNumber-1] = ubySourceAddress;
			msgprep.TxRtSaddr[m_ubyMessageNumber-1]=ubySourceSubAddress;
			msgprep.Wcnt[m_ubyMessageNumber-1] = ubyWordCount;
			msgprep.Channel[m_ubyMessageNumber-1]= ubyChannelSel;
			//msgprep.MsgTyp[m_ubyMessageNumber-1]=MsgType;
			msgprep.TrBit[m_ubyMessageNumber-1]=Tr;
			for(int i=0;i<ubyWordCount;i++)
				msgprep.Data[m_ubyMessageNumber-1][i]=0xa5a5;*/
	//***************************		BC		****************************
		...
		/*DefFrame_A(1,BCSTACK_A,m_ulmsgno,frame_items,CARD1);

		SetBCEOMCnt_A(BcMod,0,CARD1);
		RunBcNowait_A(1,BCSTACK_A,1,m_ulmsgno,1,CARD1);	
		//Sleep(10);
		while(GetBCEOMCnt_A(BcMod,CARD1)<m_ulmsgno);
				printf("\nBCEOMCount=%d\n",GetBCEOMCnt_A(BcMod,CARD1));		*/
			/*switch(ubyTimeFrequency)
			{
				case 160:
					printf("MessageName:%s \nubyMinorFrameNumber:%d MinorCycle Time:%d \n\tubySourceAddress:%d ubyDestAddress:%d",m_stMsg[m_ubyMessageNumber].szMessageName,ubyMinorFrameNumber,ubyTimeFrequency,ubySourceAddress,ubyDestAddress);
					printf("\n\tfnGetSourceSubAddress:%d ubyDestSubAddress:%d \n\tubyChannelSel:%d ubyWordCount%d\n",ubySourceSubAddress,ubyDestSubAddress,ubyChannelSel,ubyWordCount);
				break;
				case 80:
						printf("MessageName:%s \nubyMinorFrameNumber:%d MinorCycle Time:%d \n\tubySourceAddress:%d ubyDestAddress:%d",m_stMsg[m_ubyMessageNumber].szMessageName,ubyMinorFrameNumber,ubyTimeFrequency,ubySourceAddress,ubyDestAddress);
					printf("\n\tfnGetSourceSubAddress:%d ubyDestSubAddress:%d \n\tubyChannelSel:%d ubyWordCount%d\n",ubySourceSubAddress,ubyDestSubAddress,ubyChannelSel,ubyWordCount);
...
unsigned int fnHandle1553BCommunication()
{
	...
	//while(pcl1553BSimAndMon->vstFlags.bSchedulerRunning == TRUE)
	{
		...
		for (int wMinorFrameIndex = MINOR_CYCLE_ZERO; wMinorFrameIndex <=MINOR_CYCLE_SEVEN; wMinorFrameIndex++)
		{
			...
			{
				pcl1553BSimAndMon->fnUpdateTimeBroadcastInfo();
			}
