	...
	if(times==0)
        ...
        { 
          ...
          count=RegRead(ModId,0x1,1);
           while (count != 0x54c)//0x54c Completed
           {
...
int main(void)
{
    ...
    unsigned short inter_msg_gap = 10;
    ...
    for (msg_no = 1; msg_no <= msg_count; msg_no++)
    {
        ...
        if (msg_no <= 100)
        {
            ...
            DefMsg1553(module_id, msg_no, bc_page, data_buffer, 32, inter_msg_gap);
        }
        else if (msg_no <= 200)
        {
        ...
        else if (msg_no >= 601 && msg_no <= 632)
        {
            ...
            DefMsg1553(module_id, msg_no, bc_page, data_buffer, datacount, inter_msg_gap);
        }
        else
        {
    ...
    if (use_frame_scheduler == 1)
    {
       ...
       //PriorityHighLow(module_id, 1, 1);
        ...
        for (idx = 0; idx < 32; idx++)
            dynamic_data[idx] = 0x1553 + idx; /* some dynamic pattern */
