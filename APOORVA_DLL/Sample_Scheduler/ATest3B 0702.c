...
int main(void)
{
    ...
    int BcMod =3 ,rtModId = 1, mtModId = 1;
    char input[32];
    fd_set set;
   ...
   //ClearRam(1);
    ...
    write_dma_and_valr_regs(0x00,0x4000);
 ...
 // second transfer
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0,0x4000 + 0x3e8);//0x3FE
  write_dma_and_valr_reg(1);
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x02,0x4000 + 0x3e8 * 0x02);//0x7FE
  write_dma_and_valr_reg(1);
  check_bit_status(1);
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x03,0x4000 + 0x3e8 * 0x03);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x04,0x4000 + 0x3e8 * 0x04);
  write_dma_and_valr_reg(1);
  check_bit_status(1);
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x05,0x4000 + 0x3e8 * 0x05);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x06,0x4000 + 0x3e8 * 0x06);
  write_dma_and_valr_reg(1);
  check_bit_status(1);
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x07,0x4000 + 0x3e8 * 0x07);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x08,0x4000 + 0x3e8 * 0x08);
  write_dma_and_valr_reg(1);
  check_bit_status(1);
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x09,0x4000 + 0x3e8 * 0x09);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x0a,0x4000 + 0x3e8 * 0x0a);
  write_dma_and_valr_reg(1);
  check_bit_status(1);
  ...
  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x0b,0x4000 + 0x3e8 * 0x0b);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x0c,0x4000 + 0x3e8 * 0x0c);
  write_dma_and_valr_reg(1);
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x0d,0x4000 + 0x3e8 * 0x0d);
  write_dma_and_valr_reg(1); 
  check_bit_status(1);

  usleep(1000);
  Write_control_registers_for_Write(1,1);
  write_dma_and_valr_regs(0x7d0 * 0x0e,0x4000 + 0x3e8 * 0x0e);
  write_dma_and_valr_reg(1);
  check_bit_status(1);*/
    
   /* for(int i = 0x0 ; i <  0x3ffff ; i++)
    {
         printf("AddressWrite 0x%08x  Data = 0x%04x\n",i,MemWrite(2,i,0xab00+i,1));
    }*/
  //usleep(1000);
   ...
   printf("***********************reading data************************\n");
        ...
        /*for(int i = 0x0 ; i <  0x1000 ; i++)
          {
             printf("AddressRead 0x%08x  Data = 0x%04x\n",0x4000+i,MemRead(1, 0x4000+i,1));
        ...
        switch (testNum)
        {
           ...
           case 3: // RT Mode
            ...
            case 4: // BC + MT Mode
                ...
                ActivateMc(BcMod,BC);           // BC ACTIVATE
                BCSchedule_Bus(NoOfTimes, BcMod,channel); // MESSAGE PREPARATION ,FRAMING AND SCHEDULING
          ...
          case 8: // BC MT Mode    
                ...
                RegWrite(mtModId,0x3,0x2,1);  //START REGISTER
               //pthread_create(&mtThread, NULL, StartMTThread, &mtModId);*/
             //   MTConfigure(rtModId,0x5000,0x8000);
                //MTConfigure(mtModId,0xdf8f,0xa000);
               ClearRam(mtModId);
               // ConfigureRT(mtModId, rtAddr); 
                //pthread_create(&thread, NULL, RTThread_Bus, &mtModId);
              BCSchedule_NEW(NoOfTimes, mtModId,channel);
                getchar();getchar();
                for(int i=0;i<1000;i++)
                {
                    printf("Mem add=%08x  Mem data=%04x\n",0x24000+i,MemRead(mtModId,0x24000+i,1));
                }
                printf("Reg add=%08x  Reg data=%04x\n",0x1,RegRead(mtModId,0x1,1));
                printf("Reg add=%08x  Reg data=%04x\n",0x7,RegRead(mtModId,0x7,1));
                getchar();
                RegWrite(mtModId,0x3,0x1,1);
                
                //ActivateMc(BcMod,BCMT);
            ...
            case 9: // RT MT Mode
                ...
                enableNonCanonicalMode();
                 for(i=0; i < 128; i++)    //SELMESG MT LOOK-UP-TABLE
                  {
                      MemWrite(rtModId,(0x20280+i),0xffff,1); ///changed add 20280 29/01/26
                  }
                MemWrite(rtModId,(0x20102),0x1000,1); //COMMAND STACK POINTER
                MemWrite(rtModId,(0x20103),0x4000,1); //DATA STACK POINTER
                RegWrite(rtModId,0x1,0xd000,1);  //CONFIGURATION 12-BIT MUST BE HIGH
                RegWrite(rtModId,0x7,0xa000,1); //CONFIGURATION  15-BIT MUST BE HIGH
                RegWrite(rtModId,0x3,0x2,1);  //START REGISTER
                pthread_create(&mtThread, NULL, StartMTThread, &rtModId);
               //getchar();
               // RegWrite(rtModId,0x03, 0x1, 1);
                ConfigureRT(rtModId, rtAddr); 
                pthread_create(&thread, NULL, RTThread_Bus, &rtModId);
                break;
...
void ConfigureRT(UCHAR rtModId, unsigned char rtAddr)
{
 ...
 //   ResetAce(rtModId);    
    ...
    // Enable RT mode
    //DefMode(rtModId, RT);
    RegWrite(rtModId,CONFIG_REG,0xdf8f,1);
    RegWrite(rtModId,CONFIG_REG3,0xa000,1);
...
void BCSchedule_Bus(int Times, unsigned char module_id, unsigned char channel)
{
  ...
  {    
             ...
             /* ==================== FRAME 1 MESSAGE DEFINITION ==================== */
              for (msg_no = 1; msg_no <= 47; msg_no++)
              {
                  ...
                  switch (msg_no)
                  {
                      ...
                      /* ========== RT-RT #2 ========== */
                      case 31: bc_page.cmd_1.taddr = 8;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
            ...
            for (msg_no = 1; msg_no <= 41; msg_no++)
            {
               ...
               if (channel == 1)
                ...
                switch (msg_no)
                {
                    ...
                    /* ---- RT-to-RT ---- */
                    case 20: //bc_page.cmd_1.taddr = 9;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                        bc_page.rt_rt = YES;
             ...
             /* ================= FRAME 3 ================= */
              for (msg_no = 1; msg_no <= 42; msg_no++)
              {
                  ...
                  switch (msg_no)
                  {
                      ...
                      /* ---- RT-RT ---- */
                      case 16: //bc_page.cmd_1.taddr = 10;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                          bc_page.cmd_1.taddr = 15;  // TX RT
            ...
            for (msg_no = 1; msg_no <= 43; msg_no++)
            {
               ++MsCoun;
                ...
                switch (msg_no)
                {
                    ...
                    case 19: //bc_page.cmd_1.taddr = 11;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                    ...
                    case 27: bc_page.cmd_1.taddr = 7;  bc_page.cmd_1.subaddr = 1;  bc_page.cmd_1.wcnt = 17; bc_page.cmd_1.t_r = RECEIVE; break;
                    case 28: //bc_page.cmd_1.taddr = 11;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
            ...
            for (msg_no = 1; msg_no <= 45; msg_no++)
            {
               ++MsCoun;
                ...
                switch(msg_no)
                {
                    ...
                    case 13: //bc_page.cmd_1.taddr = 12;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                          bc_page.cmd_1.taddr = 15;  // RX RT
                    ...
                    case 22:// bc_page.cmd_1.taddr = 13;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                          bc_page.cmd_1.taddr = 13;  // RX RT
      ...
      break;	
            ...
            for (msg_no = 1; msg_no <= 47; msg_no++)
            {
                ...
                switch(msg_no)
                {
                    ...
                    case 36: //bc_page.cmd_1.taddr = 14;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                          ...
                          bc_page.cmd_1.t_r = RECEIVE;
                          bc_page.cmd_2.t_r = TRANSMIT;break;
                    case 37: //bc_page.cmd_1.taddr = 15;  bc_page.cmd_1.subaddr = 2;  bc_page.cmd_1.wcnt = 29; bc_page.cmd_1.t_r = RECEIVE; break;
                          bc_page.rt_rt = YES;
                          bc_page.cmd_1.taddr = 2;  // RX RT
                          bc_page.cmd_1.subaddr = 16; // RX SA
	...
	DefMajorFrame(module_id, 0, 8, major_frames);
        ...
        getchar();
        printf("Reg add=%08x  Reg data=%04x\n",0x1,RegRead(module_id,0x1,1));
        printf("Reg add=%08x  Reg data=%04x\n",0x7,RegRead(module_id,0x7,1)); 
...
void* StartMTThread(void* arg)
{
    ...
    last =((MemRead(MTModId, 0x20102, 1)-0x1000)/4);//MemRead(MTModId, 0x20102, 1);// ReadMsgMtCounter(MTModId);
    printf("last =%04x %d\n",last,MTModId);
    ...
    while (ThreadRunFlag == 0)
    {
        current = ((MemRead(MTModId, 0x20102, 1)-0x1000)/4);// ReadMsgMtCounter(MTModId);
...
void* StartMTThread(void* arg)
{
    ...
    signal(SIGINT, handle_sigint); 
    last = MemRead(MTModId,  0x20102, 1);//MemRead(MTModId, 0x102, 1);ReadMsgMtCounter(MTModId);
    //RegRead(MTModId, 0xB, 1); 
    gettimeofday(&prev_tv, NULL);

    while (ThreadRunFlag == 0)
    {
        current = MemRead(MTModId,  0x20102, 1);//ReadMsgMtCounter(MTModId); 
        //RegRead(MTModId, 0xB, 1);
...
#endif
 void BCSchedule_NEW(int Times, unsigned char module_id, unsigned char channel)
{
    struct timeval start, stop;
    long long llTimeDiff = 0,counter=0;
    int globalIncre=0,PreC=0;
  ...
  for(MC_minor_cycle_number=0;MC_minor_cycle_number<1;MC_minor_cycle_number++)
  {    
    switch(MC_minor_cycle_number)
          {
            case 0 :	       
        /* ================= FRAME 0 ================= */
              for (msg_no = 1; msg_no <=20; msg_no++)
              {
                  ++MsCoun;         
                  memset(&bc_page, 0, sizeof(bc_page));
                  if (channel == 1)
      ...
      break;	
            ...
            default :break;
             ...
             int idx;         
              ...
              major_frames[MC_minor_cycle_number].Mess_Count      = 3;
              major_frames[MC_minor_cycle_number].MinorFrame_Time = 20;
              prevPos = MsCoun;              
         }	
	DefMajorFrame(module_id, 0, 1, major_frames);
        //PriorityHighLow(module_id, 2, 1);
        printf("\tBC Scheduling Started Wait........! \n");
        RunBcMajorFrame_NEW(module_id, 0,1, major_frames, Times);
        ...
        for(int i=0;i<100;i++)
        {
            printf("Mem add=%08x  Mem data=%04x\n",0x24000+i,MemRead(module_id,0x24000+i,1));
        }
