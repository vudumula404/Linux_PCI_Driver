...
/*****************************************************************************
 * Single BC→RT Message Test
 * - BC sends 1 message per minor frame to RT address (user configurable)
 * - MT monitors and logs the message
 *****************************************************************************/
...
/* ── Ring-buffer constants (confirmed from hardware) ── */
...
volatile int             ThreadRunFlag  = 0,stop_mt=0;
...
void  BCSchedule_Bus_Single(int Times, unsigned char module_id, unsigned char channel,
...
void BCSchedule_Bus_Single(int Times,
                            ...
                            unsigned char word_count)
{
    ...
    /* Fill data words with incrementing values */
    for (idx = 0; idx < word_count; idx++)
        data_buffer[idx] = ++data_value;

    /* Define message slot 1 */
    DefMsg(module_id, i, bc_page, data_buffer, word_count, inter_msg_gap);

    /* ── Build ONE minor frame containing that 1 message ── */
    major_frames[0].frame_items[i]  = 1+i;          /* message number 1 */
    major_frames[0].Mess_Count      = 1+i;
    major_frames[0].MinorFrame_Time = 20;

    printf("Frame[0]: MsgCount=%d  Time=20  RT=%u  SA=%u  WC=%u\n",major_frames[0].Mess_Count+i,
           rt_addr, sub_addr, word_count);
    }
...
/* =========================================================================
 * StartMTThread  –  fixed ring size + proper drain on bc_done
 * ======================================================================= */
 #if 1
void* StartMTThread(void* arg)
{
    ...
    while (ThreadRunFlag == 0)
    {
        ...
        else
        {
           ...
           {
              printf("bc done but only got %lu \n",msgCount);
              break;
           }
    }
...
{
    MTThreadArgs  *args    = (MTThreadArgs*)arg;
    unsigned char  MTModId = args->BCModId;
        ...
        if (current != last)
        {
            ...
            while (diff--)
            {
                ...
                if (select(STDIN_FILENO + 1, &readfds, NULL, NULL, &tv) > 0)
                {
                    char ch;
                    read(STDIN_FILENO, &ch, 1);
                    printf("\nKey pressed → Immediate stop inside processing\n");
                    stop_mt = 1;
                    goto done;
                }
...
#endif
/* =========================================================================
 * DefMajorFrame_new  –  unchanged from original
 * ======================================================================= */
...
/* =========================================================================
 * RunBcMajorFrame_new  –  unchanged from original
 * ======================================================================= */
...
int main(void)
{
    ...
    /* ================================================================
     * MAIN MENU  –  only BC+MT mode kept (option 4)
     * Other options removed as per request
     * ============================================================== */
    while (1)
    {
        printf("\n1.BC+MT (Single BC->RT Msg)   0.Exit\n");
        scanf("%d", &choice);
        ...
        switch (choice)
        {
            ...
            case 1:
                ...
                /* Configure BC+MT hardware */
                BC_MT_Asyn(BcMod, mtModId, channel, NoOfTimes);
                ...
                /* MT expects exactly NoOfTimes messages (1 msg per frame) */
               ...
               mtArgs.expectedMessages = (unsigned long)NoOfTimes;
                ...
                /* Run BC */
                BCSchedule_Bus_Single(NoOfTimes, BcMod, channel,
                                      (unsigned char)rt_addr,
                                      (unsigned char)sub_addr,
                                      (unsigned char)word_count);
                bc_done = 1;
