...
int main(void)
{
    ...
    while (1)
    {
        printf("\n\n\t*********** APOORVA 1553B BC(As per HAL Scheduler)/RT/MT CARD ATP ***********\n");
        printf("\n\tSelect Mode:\n");
...
void BCSchedule_Bus(int Times, unsigned char module_id, unsigned char channel)
{
    ...
    unsigned short inter_msg_gap=10,msg_no=0,idx=0,data_value=0,Wcount=0;
    double dftDuration = 0.0;
    int prevPos = 0;  
    UBIT16 data[20][32], sub_addr[10], wordcnt[20];
    printf("Bus Controller Starts Scheduling...\n");
	...
	DefMajorFrame(1, 0, 8, major_frames);
        ...
        RunBcMajorFrame(1, 0,8, major_frames, Times);
        getchar();
...
void* RTThread_Bus(void* arg)
{
    ...
    lastDesc = MemRead(modId, 0x100, 1);

    while (ThreadRunFlag == 0)
    {
        currentDesc = MemRead(modId, 0x100, 1);
