...
void main()
{
	...
	WriteDataBlock(BUS_3,RT1,1,data_buffer,CARD1); 
	RunRt(BUS_3,RT1,Address,CARD1);
	
	
	Address[RT2]=RT2;
	ClearRam(BUS_3,RT2,CARD1);
	ConfigureRT(BUS_3,RT2,Address,CARD1);
	for(i=0;i<=31;i++)
		data_buffer[i]=0x1111; 		
	WriteDataBlock(BUS_3,RT2,1,data_buffer,CARD1); 
	RunRt(BUS_3,RT2,Address,CARD1);
	//***************************		BC		****************************
	...
	frame_items[0] = 1;
	DefFrame(BUS_3,BCSTACK_A,1,frame_items,CARD1);
	
	
	RunBc(BUS_3,BCSTACK_A,1,1,1,CARD1);			/*issue start Command*/
	...
	printf("\nReceive Status  Word		-->>	%x \n",msg_return.status_2);

	
	printf("\n***************Rx RT RESULTS************\n");
	ReadMsgRT(BUS_3,1,1,&RtBuff,CARD1);  
	...
	printf("\n***************Tx RT RESULTS************\n");
	ReadMsgRT(BUS_3,1,2,&RtBuff,CARD1);  
